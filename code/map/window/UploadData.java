...
/**
 * Created by computer on 2017/5/13.
 */
@SuppressWarnings("Duplicates")
public class UploadData extends JFrame implements ListSelectionListener {
    private final Object lock = new Object();

    MainWindow parent;
    UploadData this0 = this;
    User holder;
    JPanel tablePanel;
    JPanel tablePanel2;
    ...
    public UploadData(map.window.MainWindow parent, User user) throws HeadlessException {
        this.parent = parent;
        holder = user;
        {
                            ...
                            add(new JMenu("Show...") {
                                {
                                    ...
                                    add(new JMenuItem("Show on Table") {
                                        ...
                                        setTable(showCitiesOnTable(), showT());
                                    });
...
//                                    {
//                                        addActionListener(e -> {
//                                            splitLines = Splitter.split(dmp, DEFAULT_SPACE, E);
//                                            isConvert = false;
//                                            this0.repaint();
//                                        });
//                                    }
...
//            }
    ...
    private DataManager dmp = new DataManager(this0,false);
    private DataManager dmt = new DataManager(this0, false);
...

    {
        ...
        add(upPanel = new JPanel(new GridLayout(1, 2)));
...
//                            else if (map.y > 90) map.y -= 180;
                            ...
                            if (controlWhich) {
                                dmp.outAdd(p.x, p.y);
                                int last = dmp.cities.size() - 1;
                                Vector<Object> row = dmp.cities.get(last).toVector();
                                row.add(0, last);
                                tableModel.addRow(row);
                            } else {
                                dmt.outAdd(p.x, p.y);
                                int last = dmt.cities.size() - 1;
                                Vector<Object> row = dmt.cities.get(last).toVector();
                                row.add(0, last);
                                tableModelT.addRow(row);
                            }
                            ...
                            int n ;
                            if (controlWhich)
                             n = dmp.deselect(P, parser, isShowTable);
                            else   n = dmt.deselect(T, parser, isShowTable);
                            rectangle = null;
                            if (n > 0) update = true;
                             this0.repaint();
                        } else if (selectMode) {
...
//                            System.out.println("parser: " + parser);
                            int n;
                            if (controlWhich)
                                n = dmp.select(P, parser, isShowTable);
                            else n = dmt.select(T, parser, isShowTable);
                            rectangle = null;
                            if (n > 0) update = true;
                            this0.repaint();
                        }
                ...
                addMouseMotionListener(new MouseAdapter() {
                    ...
                    public void mouseDragged(MouseEvent e) {
                        ...
                        if ((deselectMode || selectMode) && start != null) {
                            ...
                            y /= marginX;
                            if (controlWhich)
                                for (int i : dmp.selected) {
                                    City c = dmp.cities.get(i);
                                    c.LNG += x;
                                    c.LAT -= y;
                                    ...
                                    if (c.LAT > 90) c.LAT -= 180;
                                    if (isShowTable){
                                        P.setValueAt(c.LAT, i, 4);
                                        P.setValueAt(c.LNG, i, 5);
                                    }
                                }
                            else
                                for (int i : dmt.selected) {
                                    City c = dmt.cities.get(i);
                                    c.LNG += x;
                                    c.LAT -= y;
                                    if (c.LNG < -180) c.LNG += 360;
                                    if (c.LNG > 180) c.LNG -= 360;
                                    ...
                                    if (c.LAT > 90) c.LAT -= 180;
                                    if (isShowTable){
                                        T.setValueAt(c.LAT, i, 4);
                                        T.setValueAt(c.LNG, i, 5);
                                    }
                                }
                            update = true;
                            this0.repaint();
...
//                        && (y1 <= _p.map.y * yd && _p.map.y * yd <= y2);
            ...
            public void drawMap(Graphics2D d, float dX, float dY) {
                ...
                pen.setColor(myColor2);
                pen.setStroke(fatterStroke);
                for (int i : dmp.selected) {
                    City city = dmp.cities.get(i);
                    float x = city.LNG * marginX + _W / 2;
                    float y = -city.LAT * marginY + _H / 2;
                    line.setLine(x, y, x, y);
                    pen.draw(line);
//                    pen.drawString("*", x, map.y);
                }
                pen.setColor(myColor);
                pen.setStroke(fatterStroke);
                for (int i : dmt.selected) {
                    City city = dmt.cities.get(i);
                    float x = city.LNG * marginX + _W / 2;
                    float y = -city.LAT * marginY + _H / 2;
                    line.setLine(x, y, x, y);
                    pen.draw(line);
                ...
                for (int i : dmp.normal) {
                    City city = dmp.cities.get(i);
                    float x = city.LNG * marginX + _W / 2;
                    float y = -city.LAT * marginY + _H / 2;
//                    line.setLine(x, map.y, x, map.y);
//                    pen.draw(line);
                    pen.drawString("*", x, y);
                }
                pen.setColor(Color.BLACK);
                pen.setStroke(fatStroke);
                for (int i : dmt.normal) {
                    City city = dmt.cities.get(i);
                    float x = city.LNG * marginX + _W / 2;
                    float y = -city.LAT * marginY + _H / 2;
                    line.setLine(x, y, x, y);
...
//                        d.drawImage(paper, 0, 0, null);
        ...
        new Thread() {
            ...
            public void run() {
                if (!isShowTable) {//两次重新展示table等价更新显示数据
                    ...
                    tablePanel.add(table[0].getTableHeader(), BorderLayout.NORTH);
                    tablePanel.add(new JScrollPane(table[0]));
                    tablePanel2.add(table[1].getTableHeader(), BorderLayout.NORTH);
                    tablePanel2.add(new JScrollPane(table[1]));
                    upPanel.add(tablePanel);
                    upPanel.add(tablePanel2, BorderLayout.EAST);
                }
                isShowTable = true;
                updateUi();
            }
    ...
    private JTable showT() {
        T.getSelectionModel().removeListSelectionListener(this0);
        clearTableData(tableModelT);
        int[] i = {0};
        dmt.forEach(city -> {
...
//            row.add(i[0]++);
//            row.add(city.id);
//            row.add(city.P_INDEX);
//            row.add(city.P_NAMES);
//            row.add(city.LAT);
//            row.add(city.LNG);
        ...
        T.getSelectionModel().addListSelectionListener(this0);
        dmt.selectTableData(T);
        return T;
    }
    JTable P = new JTable(0, StaticThing.CITY_COLUMN.length + 1){
        @Override
    ...
    JTable T = new JTable(0, StaticThing.CITY_COLUMN.length + 1){
        @Override
        public boolean isCellEditable(int row, int column) {
            if (column == 0 || column == 1) return false;
            return true;
    ...
    DefaultTableModel tableModel = (DefaultTableModel) P.getModel();
    DefaultTableModel tableModelT = (DefaultTableModel) T.getModel();
...
//        P.setSelectionMode(ListSelectionModel.MULTIPLE_INTERVAL_SELECTION);
        ...
        tableModelT.addTableModelListener(e -> {
            int r = e.getFirstRow();
            int c = e.getColumn();
            if (r < 0 || c < 0) return;
            String str = tableModelT.getValueAt(r, c).toString();
            City city = dmt.cities.get(r);
            switch (c) {
                case 2:
                    city.P_INDEX = str;
                    break;
        ...
        P.getSelectionModel().addListSelectionListener(this);
        T.getSelectionModel().addListSelectionListener(this);
    }
    {
        Enumeration<TableColumn> tableColumns = T.getTableHeader().getColumnModel().getColumns();
        final int[] i = {0};
    ...
    {
        Enumeration<TableColumn> tableColumns = P.getTableHeader().getColumnModel().getColumns();
        final int[] i = {0};
        tableColumns.nextElement().setHeaderValue("/");
        while (tableColumns.hasMoreElements()) tableColumns.nextElement().setHeaderValue(CITY_COLUMN[i[0]++]);
    }
    ...
    public JTable showCitiesOnTable() {
        P.getSelectionModel().removeListSelectionListener(this0);
        clearTableData(tableModel);
        int[] i = {0};
        dmp.forEach(city -> {
//            Vector<Object> row = new Vector<>();
...
//            row.add(city.LNG);
        ...
        P.getSelectionModel().addListSelectionListener(this0);
        dmp.selectTableData(P);
        return P;
    }

    private void clearTableData(DefaultTableModel tableModel) {
        int size = tableModel.getRowCount();
        for (int i = 0; i < size; i++) tableModel.removeRow(tableModel.getRowCount() - 1);
    }
    ...
    public void valueChanged(ListSelectionEvent e) {
        if (controlWhich)
            dmp.select(P, e.getFirstIndex(), e.getLastIndex());
        else dmt.select(T, e.getFirstIndex(), e.getLastIndex());
        update = true;
        this0.repaint();
    }
